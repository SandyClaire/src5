...
-(IBAction)readBtnClicked:(UIButton *)sender
{
    ...
    [[[CBManager sharedManager] myPeripheral] readValueForDescriptor:self.descriptor];
}
...
-(void) logOperation:(NSString *)operation andData:(NSData *)data
{
    if (data != nil)
    {
         [Utilities logDataWithService:[ResourceHandler getServiceNameForUUID:[[CBManager sharedManager] myService].UUID] characteristic:[ResourceHandler getCharacteristicNameForUUID:[[CBManager sharedManager] myCharacteristic].UUID] descriptor:[Utilities getDiscriptorNameForUUID:self.descriptor.UUID] operation:[NSString stringWithFormat:@"%@%@ %@",operation,DATA_SEPERATOR,[Utilities convertDataToLoggerFormat:data]]];
    }
    else
    {
        [Utilities logDataWithService:[ResourceHandler getServiceNameForUUID:[[CBManager sharedManager] myService].UUID] characteristic:[ResourceHandler getCharacteristicNameForUUID:[[CBManager sharedManager] myCharacteristic].UUID] descriptor:[Utilities getDiscriptorNameForUUID:self.descriptor.UUID] operation:operation];
    }

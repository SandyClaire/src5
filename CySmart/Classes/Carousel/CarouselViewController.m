...
@interface CarouselViewController () <iCarouselDataSource, iCarouselDelegate>
{
    ...
    NSMutableArray *carousel_ServiceArray;
    NSMutableArray *tempCarousel_ServiceArray;
...
-(void)viewDidLoad
{
    ...
    carousel_ServiceArray = [[NSMutableArray alloc] init];
    tempCarousel_ServiceArray = [[NSMutableArray alloc] init];
...
{
    [[CBManager sharedManager] setMyService:[carousel_ServiceArray objectAtIndex:index]] ;
...
-(void)pushViewController:(NSInteger)index
{
    ...
    NSString *keyAtIndex = [[[[CBManager sharedManager] serviceUUIDDict] allKeysForObject:carouselItem] objectAtIndex:0];
    ...
    else if ([keyID isEqual:CAPSENSE_SERVICE_UUID])
    {
        CapsenseRootVC *capsenseVC = [self.storyboard instantiateViewControllerWithIdentifier:CAPSENSE_VIEW_SB_ID];
        capsenseVC.capsenseCharList = [NSMutableArray arrayWithArray:tempCarousel_ServiceArray];
    ...
    else if ([keyID isEqual:CAPSENSE_BUTTON_CHARACTERISTIC_UUID])
    {
        ...
        CBCharacteristic *characeristic = [tempCarousel_ServiceArray objectAtIndex:0];
        buttonVc.capsenseButtonCharacteristicUUID = characeristic.UUID;
    ...
    else if ([keyID isEqual:CAPSENSE_PROXIMITY_CHARACTERISTIC_UUID])
    {
        ...
        CBCharacteristic *characteristic = [tempCarousel_ServiceArray objectAtIndex:0];
    ...
    else if ([keyID isEqual:CAPSENSE_SLIDER_CHARACTERISTIC_UUID] )
    {
        ...
        CBCharacteristic *characeristic = [tempCarousel_ServiceArray objectAtIndex:0];
        sliderVC.sliderCharacteristicUUID = characeristic.UUID;
...
-(void)prepareCarouselList
{
    ...
    if ([[CBManager sharedManager] foundServices].count == 0) {
...
-(void)checkCapsenseProfile:(CBService *)service
{
    ...
    if([service.UUID isEqual:CAPSENSE_SERVICE_UUID] || [service.UUID isEqual:CUSTOM_CAPSENSE_SERVICE_UUID])
    {
        ...
        [capsenseServiceModel startDiscoverCharacteristicWithUUID:nil withCompletionHandler:^(BOOL success, CBService *service, NSError *error)
        {
           ...
           if (success)
           {
               ...
               for (CBCharacteristic *characteristic in service.characteristics)
               {
                   ...
                   [tempCarouselArray addObject:[[[CBManager sharedManager] serviceUUIDDict] valueForKey:[keyID.UUIDString lowercaseString]]];
                   [tempCarousel_ServiceArray addObject:characteristic];
...
-(BOOL)checkFindMeProfile:(CBService *)service
{
    ...
    for(CBUUID *findMEServiceID in findMEUUIDs)
    {
        ...
        NSDictionary *fDict = [[[CBManager sharedManager] serviceUUIDDict] valueForKey:[fkeyID.UUIDString lowercaseString]];
 ...
 *
 *  @discussion Method invoked when device orientation is changed
 *
 */

-(void) deviceOrientationChanged:(NSNotification *)notification{
    
    if ([[CBManager sharedManager] foundServices].count == 0 && IS_IPAD) {
